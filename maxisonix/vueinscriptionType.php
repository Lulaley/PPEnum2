  ...
  include ("Controlleur/controlleur.class.php");
          ...
          if (isset($_POST['ValiderPart']))
        {
          if( (!empty($_POST['PrenomC'])) && (!empty($_POST['NomC'])) && (!empty($_POST['AdrC'])) && (!empty($_POST['Ville'])) && (!empty($_POST['CP'])) && (!empty($_POST['Pays'])) )
          {
            ...
            {
              if ( (!preg_match("/([^A-Za-zéèàùêâëäüï&ç -])/",$_POST['PrenomC'])) && (!preg_match("/([^A-Za-zéèàùêâëäüï&ç -])/",$_POST['NomC'])) && (!preg_match("/([^A-Za-zéèàùêâëäüï&ç0-9 -])/",$_POST['AdrC'])) && (!preg_match("/([^A-Za-zéèàùêâëäüï&ç-])/",$_POST['Ville'])) && (!preg_match("/([^A-Za-zéèàùêâëäüï&ç0-9 -])/",$_POST['AdrC'])) && (!preg_match("/([^0-9-])/",$_POST['CP'])) && (!preg_match("/([^A-Za-zéèàùêâëäüï&ç --])/",$_POST['Pays']))) 
              {
                if ( (!preg_match("/([^A-Za-zéèàùêâëäüï&ç@._0-9--])/",$_POST['MailC'])) && (!preg_match("/([^A-Za-zéèàùêâëäüï&#ç@0-9-])/",$_POST['mdpC'])) )
                {
                  InscriptionParticulierC($_POST);
        ...
        {
          include("vueinscriptionType.php");
        }

        if (isset($_POST['ValiderPro']))
        {
          if( (!empty($_POST['NomSoc'])) && (!empty($_POST['AdrC'])) && (!empty($_POST['Ville'])) && (!empty($_POST['CP'])) && (!empty($_POST['Pays']))  )
          {
            if ( (!empty($_POST['MailC'])) &&  (!empty($_POST['mdpC'])) )
            {
              if ( (!preg_match("/([^A-Za-zéèàùêâëäüï&ç -])/",$_POST['NomSoc'])) && (!preg_match("/([^A-Za-zéèàùêâëäüï&ç0-9 -])/",$_POST['AdrC'])) && (!preg_match("/([^A-Za-zéèàùêâëäüï&ç-])/",$_POST['Ville'])) && (!preg_match("/([^A-Za-zéèàùêâëäüï&ç0-9 -])/",$_POST['AdrC'])) && (!preg_match("/([^0-9-])/",$_POST['CP'])) && (!preg_match("/([^A-Za-zéèàùêâëäüï&ç --])/",$_POST['Pays']))) 
              {
                if ( (!preg_match("/([^A-Za-zéèàùêâëäüï&ç@._0-9--])/",$_POST['MailC'])) && (!preg_match("/([^A-Za-zéèàùêâëäüï&#ç@0-9-])/",$_POST['mdpC'])) )
                {
                  InscriptionProfessionnelC($_POST);
                  header ('Location: vueinscriptionValidation.php');
                }else{
                  echo "Vous avez mis des caractères interdits ! Veuillez vous référencer aux * .";
                }
...
<div class="wrapper row3">
  <main class="hoc container clear"> 
    ...
    <div class="center btmspace-80">
      ...
      <img class="resize" src='images/connexion/inscription.png'>
    </div>
    <center>
    <p> Choissiez votre situation </p>
    <form method="post" action="">
      <input type="submit" name="Particulier" value="Particulier" style="width:250px">
      <input type="submit" name="Professionnel" value="Professionnel" style="width:250px">
    </form>
    </center>
    ...
    <?php
      //Particulier
        ...
        {
          include("vueinscriptionParticulier.php");       
        }
        
        //Profesionnel
        if (isset($_POST['Professionnel']))
        {
          include("vueinscriptionProfessionnel.php");
        }
    ?>
